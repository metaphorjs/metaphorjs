...
"string"==typeof this.model?this.model=MetaphorJs.create(this.model):MetaphorJs.is(this.model,"MetaphorJs.data.Model")||(this.model=MetaphorJs.create("MetaphorJs.data.Model",this.model));if(a||d.url)this.model.store.load=a||d.url;!this.local&&this.autoLoad?this.load():c&&($.isArray(c)?this.loadArray(c):this.loadAjaxData(c));this.local&&(this.loaded=!0)},getId:function(){return this.id},isLoaded:function(){return this.loaded},isLocal:function(){return this.local},setLocal:function(a){this.local=a?
